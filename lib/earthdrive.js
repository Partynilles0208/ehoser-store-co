...
function mountEarthDrive(app, { readAuthUser, getProfile, env = process.env, fetchImpl = fetch, now = Date.now }) {
  ...
  route('config', async (req, res) => {
    ...
    res.json({
      ...
      // The public OSM raster endpoint can block browser traffic without a
      // product-specific User-Agent. CARTO renders the same OSM data and is a
      // safer browser default; EARTHDRIVE_OSM_TILES can still override it.
      osmTileUrl: env.EARTHDRIVE_OSM_TILES || 'https://a.basemaps.cartocdn.com/light_all/{z}/{x}/{y}.png',
      osmAttribution: '© OpenStreetMap contributors © CARTO',
