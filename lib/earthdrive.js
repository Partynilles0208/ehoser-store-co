...
function mountEarthDrive(app, { readAuthUser, getProfile, env = process.env, fetchImpl = fetch, now = Date.now }) {
  ...
  route('config', async (req, res) => {
    ...
    res.json({
      google3d: Boolean(env.GOOGLE_MAPS_API_KEY), expiresAt: req.earthDrive.expiresAt,
      tileUrl: '/api/earthdrive/tiles?path=%2Fv1%2F3dtiles%2Froot.json',
  ...
  route('tiles', async (req, res) => {
    ...
    if (!env.GOOGLE_MAPS_API_KEY) throw fault(503, 'Die fotorealistische Welt ist noch nicht eingerichtet.');
    const url = tileUrl(req.query.path, env.GOOGLE_MAPS_API_KEY);
