...
// ── Proxy Browser ─────────────────────────────────────────────────────────────
let _proxyHistory = [];
let _proxyHistoryIndex = -1;
let _proxyIframeTimer = null;

function proxySetStatus(msg) {
...
function proxySetLoading(loading, msg) {
    const spinner = document.getElementById('proxyLoadingSpinner');
    const msgEl = document.getElementById('proxyLoadingMsg');
    if (!spinner) return;
    spinner.style.display = loading ? 'flex' : 'none';
    if (msg && msgEl) msgEl.textContent = msg;
}

function proxyShowStart() {
    const start = document.getElementById('proxyStart');
    const iframe = document.getElementById('proxyIframe');
    const content = document.getElementById('proxyProxyContent');
...
function proxyUpdateNavButtons() {
    const back = document.getElementById('proxyBackBtn');
    const fwd = document.getElementById('proxyFwdBtn');
    if (back) back.disabled = _proxyHistoryIndex <= 0;
    if (fwd) fwd.disabled = _proxyHistoryIndex >= _proxyHistory.length - 1;
}

function proxySetUrl(url) {
    const input = document.getElementById('proxyUrlInput');
    if (input) input.value = url;
    const lock = document.getElementById('proxyLock');
    if (lock) lock.textContent = url.startsWith('https') ? String.fromCodePoint(0x1F512) : String.fromCodePoint(0x1F513);
}

function proxyNavigate(event) {
    event && event.preventDefault();
    const raw = document.getElementById('proxyUrlInput')?.value?.trim();
    if (!raw) return;
    proxyLoad(raw);
}
...
function proxyLoad(rawUrl) {
    ...
    _proxyHistoryIndex = _proxyHistory.length - 1;
    proxyUpdateNavButtons();
    proxySetUrl(url.toString());
    proxyTryIframe(url.toString());
}

function proxyTryIframe(url) {
    const iframe = document.getElementById('proxyIframe');
    const start = document.getElementById('proxyStart');
    const content = document.getElementById('proxyProxyContent');
    if (!iframe) return;
    clearTimeout(_proxyIframeTimer);
    proxySetLoading(true, 'Seite wird geladen...');
    if (start) start.style.display = 'none';
    if (content) content.style.display = 'none';
    iframe.style.display = 'none';
    iframe.src = url;

    _proxyIframeTimer = setTimeout(function() {
        var blocked = false;
        try {
            var loc = iframe.contentDocument && iframe.contentDocument.location && iframe.contentDocument.location.href;
            if (!loc || loc === 'about:blank') blocked = true;
        } catch(e) { blocked = true; }
        if (blocked) proxyFallbackToServer(url);
    }, 4000);
    ...
    iframe.onload = function() {
        ...
        if (blocked) { proxyFallbackToServer(url); return; }
        proxySetLoading(false);
        iframe.style.display = 'block';
        proxySetStatus('Direkt geladen (iframe)');
    };

    iframe.onerror = function() {
        clearTimeout(_proxyIframeTimer);
        proxyFallbackToServer(url);
    };
}

async function proxyFallbackToServer(url) {
    const iframe = document.getElementById('proxyIframe');
    const content = document.getElementById('proxyProxyContent');
    if (iframe) iframe.style.display = 'none';
    proxySetLoading(true, 'iframe blockiert - lade ueber Server-Proxy...');
    try {
        const res = await fetch('/api/proxy?url=' + encodeURIComponent(url), { cache: 'no-store' });
        if (!res.ok) {
            const err = await res.json().catch(function() { return {}; });
            proxySetLoading(false);
            proxySetStatus('Fehler: ' + (err && err.error ? err.error : res.statusText));
            if (content) {
                content.style.display = 'block';
                content.innerHTML = '<div style="padding:32px;text-align:center;color:#b63f2d;font-family:sans-serif;"><h3>Seite nicht erreichbar</h3><p>' + (err && err.error ? err.error : 'Unbekannter Fehler') + '</p><p style="margin-top:16px"><a href="' + url + '" target="_blank" rel="noopener noreferrer" style="color:#0e8a9b">Im neuen Tab oeffnen</a></p></div>';
            }
            return;
        }
        ...
        if (contentType.includes('text/html')) {
            ...
            if (content) {
                content.style.display = 'block';
                content.innerHTML = '<div style="padding:32px;text-align:center;font-family:sans-serif;"><p>Kein HTML-Inhalt (' + contentType + ').</p><p><a href="' + url + '" target="_blank" rel="noopener noreferrer" style="color:#0e8a9b">Direkt oeffnen</a></p></div>';
            }
        }
        proxySetLoading(false);
        proxySetStatus('Via Server-Proxy geladen');
    } catch(err) {
        proxySetLoading(false);
        if (content) {
            content.style.display = 'block';
            content.innerHTML = '<div style="padding:32px;text-align:center;color:#b63f2d;font-family:sans-serif;"><h3>Netzwerkfehler</h3><p>' + (err && err.message ? err.message : 'Seite nicht erreichbar') + '</p><p style="margin-top:16px"><a href="' + url + '" target="_blank" rel="noopener noreferrer" style="color:#0e8a9b">Im neuen Tab oeffnen</a></p></div>';
        }
    }
}

function proxyBack() {
    if (_proxyHistoryIndex <= 0) return;
    _proxyHistoryIndex--;
    proxyUpdateNavButtons();
    var url = _proxyHistory[_proxyHistoryIndex];
    proxySetUrl(url);
    proxyTryIframe(url);
}

function proxyForward() {
    if (_proxyHistoryIndex >= _proxyHistory.length - 1) return;
    _proxyHistoryIndex++;
    proxyUpdateNavButtons();
    var url = _proxyHistory[_proxyHistoryIndex];
    proxySetUrl(url);
