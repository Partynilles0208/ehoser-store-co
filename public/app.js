...
let worldGeneratorReady = false;
let worldTimerId = null;
let worldSecondsLeft = WORLD_LIMIT_SECONDS;
let worldPlayer = { x: 50, y: 50 };
const worldKeys = new Set();

function initWorldGenerator() {
    if (worldGeneratorReady) return;
    worldGeneratorReady = true;

    const imageInput = document.getElementById('worldImage');
    const generateButton = document.getElementById('worldGenerateBtn');
    const exitButton = document.getElementById('worldExitBtn');
    const canvas = document.getElementById('worldCanvas');

    imageInput?.addEventListener('change', () => {
        const file = imageInput.files?.[0];
        document.getElementById('worldImageName').textContent = file ? file.name : 'Optionales Referenzbild';
    });
    generateButton?.addEventListener('click', generateWorld);
    exitButton?.addEventListener('click', endWorldGenerator);
    ...
    canvas?.addEventListener('keyup', (event) => worldKeys.delete(event.key.toLowerCase()));
    canvas?.addEventListener('blur', () => worldKeys.clear());
    requestAnimationFrame(updateWorldPlayer);
}

function worldFileAsDataUrl(file) {
...
async function generateWorld() {
    const prompt = document.getElementById('worldPrompt')?.value.trim();
    const imageFile = document.getElementById('worldImage')?.files?.[0];
    const status = document.getElementById('worldStatus');
    const button = document.getElementById('worldGenerateBtn');
    if (!prompt) {
...
function startWorldTimer() {
    clearInterval(worldTimerId);
    worldTimerId = setInterval(() => {
        worldSecondsLeft -= 1;
        updateWorldTimer();
        if (worldSecondsLeft <= 0) endWorldGenerator();
    }, 1000);
}

function updateWorldTimer() {
    document.getElementById('worldTimer').textContent = `${Math.max(0, worldSecondsLeft)}s`;
    document.getElementById('worldQuota').textContent = `${Math.max(0, worldSecondsLeft)}s verfügbar`;
}

function updateWorldPlayer() {
    const speed = 0.7;
    if (!document.getElementById('worldGame')?.hidden) {
        if (worldKeys.has('w')) worldPlayer.y -= speed;
        if (worldKeys.has('s')) worldPlayer.y += speed;
        if (worldKeys.has('a')) worldPlayer.x -= speed;
        if (worldKeys.has('d')) worldPlayer.x += speed;
        worldPlayer.x = Math.max(3, Math.min(97, worldPlayer.x));
        worldPlayer.y = Math.max(5, Math.min(93, worldPlayer.y));
        const player = document.getElementById('worldPlayer');
        if (player) {
            player.style.left = `${worldPlayer.x}%`;
            player.style.top = `${worldPlayer.y}%`;
        }
    }
    requestAnimationFrame(updateWorldPlayer);
}

function endWorldGenerator() {
    clearInterval(worldTimerId);
    worldTimerId = null;
    worldKeys.clear();
    document.getElementById('worldGame').hidden = true;
    document.getElementById('worldSetup').hidden = false;
    document.getElementById('worldGenerateBtn').disabled = false;
    document.getElementById('worldStatus').textContent = worldSecondsLeft <= 0 ? 'Die 30 Sekunden sind aufgebraucht.' : 'Welt beendet.';
    worldSecondsLeft = WORLD_LIMIT_SECONDS;
    updateWorldTimer();
